...
def student_class_page(class_id):
    ...
    return render_template(
        ...
        quizzes=get_class_quiz(class_id),
        grades=get_student_grade(class_id),
    )
...
def student_quiz_page(quiz_id):
    ...
    if result == None:
        questions_db = query_db(
            "SELECT id, question_text, a_answer_text, b_answer_text, c_answer_text, d_answer_text FROM questions WHERE quiz_id=?;",
            [quiz_id],
        )
        questions = []
