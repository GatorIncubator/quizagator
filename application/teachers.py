...
def class_page(class_id=None):
    ...
    return render_template(
        ...
        topics=get_class_topic(class_id),
        assignments=get_class_assign(class_id),
        students=get_students_class(class_id),
        grades=get_class_grades(class_id),
    )
...
def topic_page(topic_id=None):
    ...
    return render_template(
        ...
        assignments=get_topic_assign(topic_id),
        topic_name=str(topic_data[0]),
        quizzes=get_topic_quiz(topic_id),
    )
...
def assignments_page():
    return render_template(
        ...
        topics=get_teacher_topic_all(),
        assignments=get_teacher_assign(),
    )
...
def create_assignment():
    ...
    insert_db(
        "insert into assignments (name, description, due_date, topic_id) values (?, ?, ?, ?);",
        [
            request.form["name"],
...
def quiz_page(quiz_id=None):
    questions_db = query_db(
        "select question_text, correct_answer, a_answer_text, b_answer_text, c_answer_text, d_answer_text from questions where quiz_id=?;",
        [quiz_id],
    )
    questions = []
...
def create_question(quiz_id=None):
    insert_db(
        "INSERT INTO questions (correct_answer, question_text, a_answer_text, b_answer_text, c_answer_text, d_answer_text, quiz_id) VALUES (?, ?, ?, ?, ?, ?, ?);",
        [
            str(request.form["answer"]),
            str(request.form["question"]),
            str(request.form["a_answer"]),
            str(request.form["b_answer"]),
            str(request.form["c_answer"]),
            str(request.form["d_answer"]),
            str(quiz_id),
        ],
    )
...
def create_grade(class_id):
    insert_db(
        "INSERT INTO assignment_grades (student_id, assignment_id, grade) VALUES (?, ?, ?);",
